...
function createComment(url, request) {
  ...
  const response = {};
  if (requestComment && requestComment.username && database.users[requestComment.username] && requestComment.articleId && requestComment.body && requestComment.articleId
...
function loadDatabase() {
  fs.readFile('./database.yaml', "utf-8", function read(err, data) {
      if(err){
        throw err;
      }
      if(data){
...
function saveDatabase(){
  fs.writeFile('./database.yaml', JSON.stringify(database, null, 4), function(err) {
    if(err){
      throw err;
    }else{
